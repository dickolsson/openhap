...
=item C<pv> - Protocol version (1.1)
...
process that executes:
    ...
    mdnsctl publish "Service Name" hap tcp port "key=value,..."

The C<mdnsctl publish> command runs indefinitely to maintain the service
registration. The process is properly daemonized using C<setsid(2)> and
its PID is stored for cleanup during unregistration.
...
Unregister the HAP service by terminating the background C<mdnsctl>
process. Sends C<SIGTERM> to the child process and reaps it with
C<waitpid(2)>.

Returns C<1> on success. This should be called during daemon shutdown
to clean up the service registration.

If the service is not registered, this method returns immediately.

This method is also called automatically by the C<DESTROY> method when
