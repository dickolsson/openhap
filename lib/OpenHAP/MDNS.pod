...
Register the HAP service with mdnsd via mdnsctl. Executes:

    mdnsctl proxy add _hap._tcp "Service Name" port "key=value" ...
...
Unregister the HAP service from mdnsd. Executes:

    mdnsctl proxy del _hap._tcp "Service Name"

Returns C<1> on success, C<undef> on failure. This should be called
during daemon shutdown to clean up the service registration.
