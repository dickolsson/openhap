...
2. **Null values**: If a characteristic is not readable or has no current
   value, `value` may be omitted or null.

3. **Custom characteristics**: Vendors can define custom characteristics
   using UUIDs outside the Apple base range.

4. **Value constraints**: `valid-values` and `valid-values-range` constrain
   the allowed values for enumerated types.

5. **Units**: The `unit` field is informational; the actual value is always
   in the native unit (Celsius for temperature, etc.).
